...
- repository GitHub con codice Terraform, manifest Kubernetes, Dockerfile e buildspec
- diagramma architetturale (anche disegnato a mano o con uno strumento a scelta)
- README del repository con istruzioni di deploy da zero e destroy/cleanup
- evidenza di una pipeline riuscita almeno una volta
- evidenza dei Pod in stato Running/Ready
- evidenza che l’aggiornamento v2 di ops-dashboard è stato distribuito dalla pipeline
- documentazione di almeno un problema risolto durante lo sviluppo

---

## 11. Demo finale

Cosa dovete far vedere:

- infrastruttura esistente: ECR, cluster EKS, `kubectl get all` con pod Running/Ready
- chiamata reale alle API e verifica che la segnalazione compaia nel cruscotto
- v2 di ops-dashboard in produzione, arrivata tramite pipeline
- rollout history con almeno due versioni
- endpoint `/api/stats` funzionante
- troubleshooting su un problema reale osservato durante la demo
...
kubectl get all -n prod
kubectl get pods -n prod
kubectl rollout history deployment/ops-dashboard -n prod
curl http://<ops-dashboard-url>/api/stats
kubectl logs -n prod deploy/ops-dashboard
```
...
SentinelGrid dimostra un’architettura moderna su AWS EKS con containerizzazione, automazione Terraform, pipeline CI/CD, storage persistente e gestione dello stato applicativo in Kubernetes. Il progetto è stato progettato per essere facilmente deployabile, facilmente diagnostico e adatto a una presentazione tecnica completa in ambito DevOps / Cloud Native.
